...
void VectorMap::AddLine(const Vector2f& p0, const Vector2f& p1) {
  lines.push_back(Line2f(p0, p1));
}

bool VectorMap::Save(const string& file) {
  ScopedFile fid(file.c_str(), "w", true);
  if (fid() == nullptr) {
  ...
  printf("Saved vector map %s with %d lines\n",
      file.c_str(), static_cast<int>(lines.size()));
  return true;
}
