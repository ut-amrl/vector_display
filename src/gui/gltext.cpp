...
void GLText::initializeGlyph(QChar ch) {
  ...
  if (debug) {
    printf("Glyph for QChar 0x%04hX, '%s' Width %f, %d polygons, %d vertices\n",
           ch.unicode(), QString(ch).toStdString().c_str(), glyph.width,
           polygons.size(), numVertices);
  }
  ...
  if (debugTesselation) {
    printf("\nBegin tesselation of character %s\n",
           QString(ch).toStdString().c_str());
  }
